...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory contract and live read-only interface for lifecycle state, autostart, allocations, snapshots, and logical attachments. An idempotent exact-confirmation API now delegates start, stop, restart, and force-stop through the guarded lifecycle helper. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside responses; interface controls and other VM mutations remain.
