...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory and live controls including validated logical snapshots and boot order, plus idempotent exact-confirmation actions for power, offline snapshots, cloning, recoverable deletion, offline CPU/memory/boot updates, and offline managed image/network attachments. All delegate through guarded helpers with task/audit publication. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside responses; create, passthrough, and console mutations remain.
