...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  - Apps and Backup publish bounded privacy-safe state through authenticated, schema-defined v1 routes, and both interface areas render authenticated live, empty, and independently unavailable states. Backup plans now support guarded, idempotent “run now” and latest-snapshot restore controls with exact confirmation and server-confirmed refresh. Registry/digest, filesystem/repository, credential, and snapshot identifiers remain outside the API service boundary; backup creation and application mutations remain.
