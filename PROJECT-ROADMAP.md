...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  - Apps and Backup publish bounded privacy-safe state through authenticated, schema-defined v1 routes, and both interface areas render authenticated live, empty, and independently unavailable states. Applications now expose verified running state plus guarded, idempotent start/stop and data-preserving removal controls; backup plans support guarded “run now” and latest-snapshot restore controls. Every mutation requires exact confirmation and server-confirmed refresh. Registry/digest, container internals, filesystem/repository, credential, and snapshot identifiers remain outside the API service boundary; backup creation and application install/update remain.
