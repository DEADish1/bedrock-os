...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory contract and live read-only interface for lifecycle state, autostart, allocations, snapshots, and logical attachments. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside the response; authenticated mutations remain.
  ...
  - Remote Access now renders the authenticated privacy-safe trusted-device feed with active, expired, revoked, empty, disconnected, and unavailable states. Fabricated pairing, identity, device, and client-download interactions were removed; authenticated pairing/revocation mutations and signed desktop clients remain.
  - Settings now has a dedicated authenticated read contract for the validated update-check choice, stable/beta channel, enforced manual installation, and telemetry-disabled state. Hostname, account, network, credential, and other first-run configuration remain outside the response; the live interface and authenticated mutations remain.
