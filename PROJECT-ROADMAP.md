...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory contract and live interface for lifecycle state, autostart, allocations, snapshots, logical attachments, and exact-confirmation start, stop, restart, and force-stop controls. Power actions delegate idempotently through the guarded lifecycle helper. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside responses; other VM mutations remain.
