...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory and live controls for creation, power, offline snapshots, cloning, recoverable deletion, offline CPU/memory/boot updates, and offline managed image/network attachments. The authenticated passthrough mutation route also enforces two-stage confirmation and hardware revalidation, but its purpose-specific candidate feed and UI are still pending. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside general inventory responses; console integration remains.
