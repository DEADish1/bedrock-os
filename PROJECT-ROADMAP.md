...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory plus idempotent exact-confirmation actions for creation, power, offline snapshots, cloning, recoverable deletion, offline CPU/memory/boot updates, and offline managed image/network attachments. All delegate through guarded helpers with task/audit publication. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside responses; passthrough and console mutations plus the creation interface remain.
