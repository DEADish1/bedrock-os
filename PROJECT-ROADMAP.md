...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory and live interface including validated logical snapshot names, plus exact-confirmation controls for power, offline snapshots, cloning, and recoverable deletion. All delegate idempotently through guarded helpers, with the root broker binding deletion to the current VM definition hash. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside responses; create, resource, attachment, passthrough, and console mutations remain.
