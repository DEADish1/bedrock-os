...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - The image library publishes a root-generated path-free inventory after initialization, import, and conversion, with an authenticated v1 contract and live interface for format, size, SHA-256, conversion provenance, and guarded hash-bound conversion to QCOW2 or raw images. A safe browser-to-staging upload boundary and import controls remain.
  - Storage now has a dedicated authenticated v1 read contract and live interface for capacity, SMART health, software RAID/ZFS state and rebuild progress, bounded hardware-RAID visibility, and guarded integrity scrubs. Scrub requests resolve backend and layout only at the root boundary. Device paths, serials, member patterns, controller addresses, slot identifiers, and vendor descriptions stay outside the response; create, expand, replace, export, and import interface mutations remain.
  ...
  - Users now has a dedicated authenticated contract and live interface for managed NAS accounts, credential-rotation state, group membership counts, exact-confirmation creation of fixed-policy local users and groups, and membership additions between existing managed identities. Passwords, hashes, member identities, numeric IDs, shell/home options, datasets, ACLs, share paths, and snapshots remain outside the response; root-staged credential mutation remains.
