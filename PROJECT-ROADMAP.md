...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Remote Access now renders the authenticated privacy-safe trusted-device feed with active, expired, revoked, empty, disconnected, and unavailable states. Authenticated idempotent rename, expiry, and revocation mutations are available through a strict root wrapper; revocation terminates active sessions and every change publishes task/audit state. Pairing controls and signed desktop clients remain.
