...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory and live controls for creation, power, offline snapshots, cloning, recoverable deletion, offline CPU/memory/boot updates, managed image/network attachments, and GPU/USB passthrough. A purpose-specific feed exposes only safe passthrough candidates and active assignments; the offline editor requires two-stage confirmation, and execution revalidates hardware while preventing cross-VM reuse. Running guests expose an exact-confirmation embedded noVNC console: a state-bound one-time 60-second token is carried only in the same-origin WebSocket subprotocol, atomically redeemed through the root broker, stripped before a bounded loopback-only proxy, and never persisted in the broker ledger or placed in a URL. Real-guest browser acceptance remains. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside general inventory responses.
  ...
  - Settings now has a dedicated authenticated read contract and live interface for the validated update-check choice, stable/beta channel, enforced manual installation, telemetry-disabled state, and guarded outbound relay provisioning. Relay setup and disablement use strong state preconditions, idempotency, exact confirmation, and a root-only staging boundary; the response exposes only configured/not-configured state and never the endpoint, route, or token. Hostname, account, network, credential, and other first-run configuration remain outside the response.
...
- [x] Handle offline, reconnecting, partial failure, and concurrent changes.
  - Independent dashboard components degrade without collapsing the full response. API state and audit reads pin non-symlink file descriptors and reject mid-read mutation. The interface detects browser-offline and transient API failures, retains visibly labeled last-confirmed state, pauses management controls, retries on network return and while visible, and clears credentials on an explicit authorization failure. Every management mutation family requires the exact strong ETag loaded with the displayed state and rejects stale decisions for review.
- [ ] Add API schema tests, authorization tests, and UI end-to-end tests.
  - The authenticated OpenAPI 3.1 document covers every implemented v1 route, its bearer security policy, and the purpose-specific one-time WebSocket console boundary. Route/schema agreement, recursive successful-response conformance, every bearer-authenticated operation with missing and invalid credentials, token issue/revocation tests, the console upgrade relay, token non-disclosure, and initial browser console authorization now pass. Browser end-to-end coverage also verifies token entry, all management-area navigation, an authenticated idempotent settings mutation, server-confirmed refresh, and automated accessibility checks. Remaining mutation flows and real-guest console behavior still need browser coverage.
...
- [ ] Implement one-time QR/manual-code pairing with server approval and expiry.
  - A root-only backend issues QR/manual payloads, stores only code and client-key hashes, requires exact local approval, expires after ten monotonic minutes or reboot, caps attempts, and atomically rejects replay. The authenticated interface lists sanitized pending requests and performs guarded idempotent approval without exposing codes or keys. A dedicated rate-limited Unix-socket gateway gives only the isolated remote-transport identity bounded request, redemption, and exact paired-key session authorization with indistinguishable errors and secret-free process arguments; an idempotent service provisions and verifies the server X25519 identity first. A bounded connected-stream responder now performs Noise XX and binds every gateway client key to the proven remote static identity. Relay/direct ingress, longer-lived API sessions, device-target membership, and independent review remain.
- [ ] Issue per-device keys; support listing, renaming, expiry, and revocation.
  - Pairing redemption atomically registers the client's distinct public key. A root-only manager and authenticated interface provide bounded listing, renaming, expiry, and exact-confirmation revocation; a hardened collector publishes a key-free device view through the authenticated API. Session authorization matches the registered public key, rejects expired/revoked devices, records last-seen time, and starts the device's dedicated target. Authorized Noise streams run in unique hardened transient services that are `PartOf` that target, bind their authenticated device route, and receive the root-only server identity only through a systemd credential; revocation persists before synchronously stopping the target and its streams. Long-lived encrypted records forward the existing authenticated v1 API with sequence, time, byte, body, route, header, and rekey bounds. A per-session API token exists only as a systemd credential and is revoked on exit. A kernel-peer-authenticated, Unix-only launch broker now lets the unprivileged future relay connector hand off connected streams without exposing service control, root credentials, or relay-controlled parsing to a privileged network process. The outbound TLS connector and end-to-end client acceptance remain before this item closes.
