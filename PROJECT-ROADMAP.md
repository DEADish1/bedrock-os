...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Storage now has a dedicated authenticated v1 read contract and live interface for capacity, SMART health, software RAID/ZFS state and rebuild progress, bounded hardware-RAID visibility, guarded integrity scrubs, and managed export/import. Requests resolve backend and layout only at the root boundary; exported groups remain visible through a path-free managed-state summary so they can be reconnected. Device paths, serials, member patterns, controller addresses, slot identifiers, and vendor descriptions stay outside the response; create, expand, and replace interface mutations remain.
