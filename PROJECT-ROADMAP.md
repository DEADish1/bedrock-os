...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Virtual machines now have a dedicated authenticated v1 inventory and live interface including validated logical snapshot names, plus exact-confirmation controls for power and offline snapshot create/restore/delete operations. These delegate idempotently through guarded lifecycle helpers with task/audit publication. Host paths, console sockets, MAC addresses, raw definitions, and passthrough topology remain outside responses; other VM mutations remain.
