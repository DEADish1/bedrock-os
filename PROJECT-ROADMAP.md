...
- [ ] Finish Storage, VMs, Images, Apps, Connect, Backup, Hardware, Settings, and Help.
  ...
  - Users now has a dedicated authenticated contract and live interface for managed NAS accounts, credential-rotation state, group membership counts, and exact-confirmation creation of fixed-policy local users and groups. Passwords, hashes, member identities, numeric IDs, shell/home options, datasets, ACLs, share paths, and snapshots remain outside the response; membership and root-staged credential mutations remain.
