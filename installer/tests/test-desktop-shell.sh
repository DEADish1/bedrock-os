...
grep -q '"Win32_System_Pipes"' "$TAURI/Cargo.toml"
...
grep -q 'struct ProgressTracker' "$TAURI/src/write_progress.rs"
grep -q 'INSTALLER_PROGRESS_EVENT' "$TAURI/src/write_progress.rs"
grep -q 'struct PlatformPhysicalDevice' "$TAURI/src/physical_device.rs"
grep -q 'ReadFile' "$TAURI/src/physical_device.rs"
grep -q 'WriteFile' "$TAURI/src/physical_device.rs"
grep -q 'SetFilePointerEx' "$TAURI/src/physical_device.rs"
grep -q 'CloseHandle' "$TAURI/src/physical_device.rs"
grep -q 'libc::lseek' "$TAURI/src/physical_device.rs"
grep -q 'libc::close' "$TAURI/src/physical_device.rs"
...
grep -q 'target_os != "linux" && target_os != "macos" && target_os != "windows"' "$TAURI/build.rs"
...
grep -q 'BEDROCK_PHYSICAL_WRITER' "$TAURI/src/macos_service.m"
grep -q '24 \* 60 \* 60' "$TAURI/src/macos_service.m"
...
grep -q '\.stdout(Stdio::piped())' "$LIB"
grep -q 'fn relay_progress_lines' "$LIB"
grep -q 'fn write_helper_progress' "$LIB"
grep -q 'CreateNamedPipeW' "$LIB"
grep -q 'PIPE_REJECT_REMOTE_CLIENTS' "$LIB"
grep -q 'GetNamedPipeClientProcessId' "$LIB"
grep -q 'GetProcessId(info.hProcess)' "$LIB"
grep -q 'fn windows_progress_pipe_name' "$LIB"
...
grep -q 'BRProgressProtocol' "$TAURI/src/macos_service.m"
grep -q 'setInterface:progress' "$TAURI/src/macos_service.m"
grep -q 'BRProgressReceiver' "$TAURI/src/macos_service.m"
grep -q '\[progress invalidate\]' "$TAURI/src/macos_service.m"
grep -q 'fn receive_macos_progress' "$LIB"
...
grep -q 'bedrock_macos_open_exclusive_disk' "$LIB"
