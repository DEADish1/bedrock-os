...
The native media engine is implemented and tested for ISO streams and compressed raw images. It enforces the signed expanded size, emits progress, flushes the target, rereads the written range, and verifies the signed SHA-256. The finalization layer then requires platform cache synchronization before attempting eject and returns an explicit automatic-eject or safe-manual-removal result. A single-device virtual harness exercises that complete pipeline and proves checksum or synchronization failures stop at the correct boundary. Normal builds probe and close the inventory-derived whole device without writing.

Progress updates use a strict schema with the verification session, monotonic sequence, phase, completed bytes, and signed total bytes. The graphical shell listens only for the fixed native event and maps preparation, approval, writing, verification, finalization, completion, and failure to truthful labels. The in-process write pipeline produces real writing and reread counts. Linux relays those updates through a one-way standard-output pipe from the exact policy-bound helper. Windows uses a local-only inbound named pipe whose client process ID must equal the elevated process returned by the native approval launch. macOS passes a progress-only proxy object over the existing mutually code-signed XPC connection and locks its invalidation before the native callback context is released. All three transports require the expected session, signed total, next sequence, valid stage, and bounded message. Malformed display data is ignored and cannot change or stop the disk operation, while completion still requires the helper's successful final result.

Acceptance-only Linux, Windows, and macOS adapters can now keep one exclusive handle connected through the pipeline when the build uses the exact physical-writer token and production trust mode. The Windows adapter owns one zero-share `PhysicalDrive` handle for streaming, reread verification, cache flush, allow-removal, eject, and final close. The macOS adapter similarly owns one exclusive raw-disk descriptor through streaming, reread verification, full synchronization, eject, and final close. Default builds still close the probe without writing. CI compiles the gated branches but never runs them. The helper protocol reports completed writing separately from its preflight-only result.

A separate `bedrock-media-writer` executable now owns the protected-helper preflight. Before reading any request it requires effective UID 0 on Unix/macOS or an elevated Windows process token. It then reads one small, strict, time-limited request, independently re-verifies the release, finds only a packaged platform scanner relative to itself, refreshes drive inventory, and repeats the exact selection checks. Linux launches the exact policy-bound helper through `pkexec`; Windows invokes the helper directly with native `runas`; macOS 13+ registers the launch daemon through SMAppService and exchanges the bounded request over a privileged XPC connection with mutual signing-identifier and Team-ID requirements. Ordinary builds close the exclusive probe and return the dedicated preflight-only result; only exact-token builds connect the verified pipeline.
