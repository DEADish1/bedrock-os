...
Progress updates use a strict schema with the verification session, monotonic sequence, phase, completed bytes, and signed total bytes. The graphical shell listens only for the fixed native event and maps preparation, approval, writing, verification, finalization, completion, and failure to truthful labels. The in-process write pipeline produces real writing and reread counts; relaying those byte updates across the privileged Linux, Windows, and macOS boundaries is the next isolated safety checkpoint.
