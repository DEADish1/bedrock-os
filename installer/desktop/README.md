...
The native media engine is implemented and tested for ISO streams and compressed raw images. It enforces the signed expanded size, emits progress, flushes the target, rereads the written range, and verifies the signed SHA-256. The elevated helper can now probe an inventory-derived whole device with exclusive access and confirm its capacity, but it closes the handle without connecting it to this engine; physical writing remains disabled.
