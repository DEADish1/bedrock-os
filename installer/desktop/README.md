...
Progress updates use a strict schema with the verification session, monotonic sequence, phase, completed bytes, and signed total bytes. The graphical shell listens only for the fixed native event and maps preparation, approval, writing, verification, finalization, completion, and failure to truthful labels. The in-process write pipeline produces real writing and reread counts. Linux now relays those updates through a one-way standard-output pipe from the exact policy-bound helper; the app accepts only the expected session, signed total, next sequence, valid stage, and bounded line. Malformed display data is ignored and cannot change or stop the disk operation, while completion still requires the helper's successful exit. Authenticated Windows and macOS byte relays remain separate safety checkpoints.
