...
The desktop package now also contains a platform-neutral native streaming engine for the protected helper. It writes ISO or compressed raw content, reports byte progress, rejects short or oversized streams, flushes, rereads the exact signed range, and requires the signed expanded SHA-256. A separate finalization layer requires platform cache synchronization before eject and distinguishes automatic eject from safe manual removal. The combined pipeline is covered end-to-end with one in-memory virtual device so verification and finalization cannot silently target different handles. Physical connections remain gated acceptance code, not normal installer behavior.

The installer progress contract is versioned, bound to the opaque verification session, monotonic, and limited to the signed write size. The pipeline distinguishes writing, reread verification, finalization, and completion, while the interface truthfully shows local preparation and administrator-approval phases instead of a simulated percentage. Cross-process byte updates remain disabled until the protected platform transports expose authenticated read-only progress channels.
...
The interface keeps failed progress visible, never labels incomplete media as ready, and gives distinct recovery actions for insufficient space, interruption, verification failure, unavailable media, administrator approval, invalid progress, and unknown failures. USB is preferred. For optical media, see [`docs/INSTALL-MEDIA.md`](../docs/INSTALL-MEDIA.md); the installer verifies the signed ISO but does not burn CDs or DVDs.
...
The protected native helper has Linux, Windows, and macOS physical adapters that carry one exclusive device handle through write, reread verification, cache synchronization, and finalization. They are compiled only with the exact physical-writer token plus production trust mode; normal builds remain preflight-only. Windows retains the same zero-share `PhysicalDrive` handle through the entire pipeline, while macOS retains one exclusively locked raw-disk descriptor through full cache synchronization and eject. A completed write has a distinct protected-helper result and is never inferred from the preflight-only result. Linux protected writes return strictly validated, session-bound byte progress over a one-way pipe. Windows does the same through a local-only inbound named pipe authenticated against the elevated helper process ID. macOS now returns progress through a proxy object carried by its mutually authenticated XPC connection. Display-channel failure cannot affect any write, and the interface does not announce completion before the helper returns a successful final result.
