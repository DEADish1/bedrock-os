...
The protected native helper has Linux and Windows physical adapters that carry one exclusive device handle through write, reread verification, cache synchronization, and finalization. They are compiled only with the exact physical-writer token plus production trust mode; normal builds remain preflight-only, and macOS builds reject that enable request. Windows retains the same zero-share `PhysicalDrive` handle through the entire pipeline. A completed write has a distinct protected-helper result and is never inferred from the preflight-only result.
