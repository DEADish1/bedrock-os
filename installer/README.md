...
The protected native helper has a Linux-only physical adapter that can carry one exclusive handle through the verified pipeline. It is compiled only with the exact physical-writer token plus production trust mode; normal builds remain preflight-only, and Windows/macOS builds reject that enable request. A completed write has a distinct protected-helper result and is never inferred from the preflight-only result.
