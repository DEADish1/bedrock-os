...
- Added bounded authenticated API forwarding inside authorized Noise sessions. A unique short-lived local bearer token is delivered only as a service credential and revoked on exit; remote requests cannot set authorization and are constrained by route, method, header, body, sequence, age, byte, record, and deterministic rekey limits.
- Completed the Noise record header contract: protocol version, handshake-derived session identity, direction, sequence, message type, and plaintext length are all supplied as AEAD associated data and checked before dispatch, closing cross-session, reflection, type-confusion, reorder, and framing gaps.
- Added a kernel-peer-authenticated, Unix-only connected-stream broker between the future unprivileged relay connector and fixed hardened session launchers. It accepts exactly one connected stream descriptor, canonical opaque identifiers, and one of two fixed modes; pairing receives no API access, concurrency is capped, and root never parses relay-controlled stream bytes.
- Added an outbound-only TLS 1.3 relay connector with hostname verification, exact ALPN, no early data, bounded multiplexed opaque streams, strict transition handling, reconnect backoff, and local descriptor handoff. It remains inactive without root-provisioned endpoint and credentials and never opens an inbound network port.
- Added guarded relay provisioning and disablement with exact confirmation, strict endpoint/route/token validation, owner-only split configuration and credentials, secret-free results, immediate activation, stop-before-removal behavior, and rejection tests for bad confirmation and indirect requests.
...
- Added shut-off-only VM clone and recoverable-delete controls to the live interface, with validated destination names, exact confirmations, explicit storage impact, and server-confirmed refresh.
...
- Added an authenticated Users read contract exposing managed NAS account and group summaries without passwords, hashes, member identities, datasets, ACLs, share paths, or snapshot names.
- Enabled the Users management area with live account, group, and credential-rotation summaries plus explicit disconnected, unavailable, empty, and read-only states.
...
- Replaced the overview's fabricated health and capacity values with an explicit in-memory authentication flow and live `/api/v1/dashboard` rendering, including disconnected, loading, partial, unavailable, and connected states.
...
- Corrected the prototype sidebar footer so it identifies sample data and the current development version instead of presenting a fabricated live-health and uptime claim.
...
- Added an authenticated dashboard summary endpoint that exposes bounded, privacy-safe hardware, storage, alert, VM, and update telemetry with independent partial-failure reporting.
...
- Added atomic VM console token redemption with expiry/replay rejection, repeated running-guest and display-policy checks, exact Unix-socket validation, and secret-free proxy handoff.
- Added a single-client VNC WebSocket proxy boundary that consumes ten-second root-only handoffs, targets only the validated Unix socket, binds only to reserved loopback ports, and enforces connection and idle timeouts for authenticated API reverse-proxying.
- Documented signed VirtIO Windows storage/network driver handling and added a machine-readable fail-closed support gate; documented that current Bedrock does not support macOS guests on Apple or non-Apple hosts and does not distribute Apple software or bypass material.
- Added persistent software TPM 2.0 to managed VM definitions and capability reporting, preserving TPM state for recoverable deletion while removing it during failed registration or clone rollback; Windows remains gated on guest acceptance evidence.
- Added a fixed VirtIO QEMU guest-agent channel to managed VMs and enabled agent-or-ACPI graceful shutdown/restart without weakening bounded final-state verification or automatically escalating to force-stop.
- Added the privacy-safe milestone 0.5 Linux/Windows guest acceptance contract, requiring distinct physical sessions bound to one Bedrock image and verified install, acceleration, Secure Boot, TPM, agent, network, console, resource, reboot, snapshot, and restore-integrity evidence.
...
- Added guarded offline vCPU, memory, and boot-order updates with exact review phrases, host-reservation revalidation, bounded disk or CD/DVD-then-disk policy, persistent libvirt readback verification, atomic allocation/definition updates, shared locking, and rollback.
...
- Produced the first fully green short-lived physical/VM acceptance kit after two protected-writer builds, raw-image boot/rollback verification, reproducibility comparison, signed-media verification, and guarded Linux installer packaging; published the exact artifact and safe VMware, Hyper-V, Intel, AMD, USB, and system-install test order.
