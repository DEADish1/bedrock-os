...
id=12345678-1234-4123-8123-123456789abc code=BRK-ABCD-2345 boot=87654321-4321-4321-8321-cba987654321 device=42345678-1234-4123-8123-123456789abc
run() { now=$1; shift; BEDROCK_PAIRING_TEST_MODE=1 BEDROCK_PAIRING_STATE_DIR="$work" BEDROCK_PAIRING_SERVER_KEY="$work/server-public-key" BEDROCK_PAIRING_TEST_BOOT_ID="$boot" BEDROCK_PAIRING_TEST_NOW="$now" BEDROCK_PAIRING_TEST_WALL_NOW=1000 BEDROCK_PAIRING_TEST_ID="$id" BEDROCK_PAIRING_TEST_DEVICE_ID="$device" BEDROCK_PAIRING_TEST_CODE="$code" python3 "$tool" "$@"; }
...
run 103 redeem "$id" "$code" "$client" | jq -e --arg device "$device" '.status=="redeemed" and .device_id==$device and (.client_key_sha256|test("^[0-9a-f]{64}$"))' >/dev/null
run 104 authorize "$device" "$client" | jq -e --arg device "$device" '.status=="authorized" and .device_id==$device and .session_target==("bedrock-remote-device@"+$device+".target")' >/dev/null
