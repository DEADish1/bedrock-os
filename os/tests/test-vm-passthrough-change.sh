...
jq -e '[.tasks[] | select(.kind=="vm-passthrough-assign" and .state=="succeeded")] | length==1' "$work/api/tasks.json" >/dev/null
...
jq -e '[.tasks[] | select(.kind=="vm-passthrough-remove" and .state=="succeeded")] | length==1' "$work/api/tasks.json" >/dev/null
make_request assign
printf 'running\n' > "$work/runtime-state"
if run >/dev/null 2>&1; then printf 'error: running VM passthrough was accepted\n' >&2; exit 1; fi
jq -e '[.tasks[] | select(.kind=="vm-passthrough-assign" and .state=="failed")] | length==1' "$work/api/tasks.json" >/dev/null
printf 'shut off\n' > "$work/runtime-state"
jq -se '[.[] | select(.action | startswith("vm-passthrough-"))] | length==3' "$work/api/audit.jsonl" >/dev/null
