...
run_checker() {
  ...
  BEDROCK_UPDATE_CERT="$work/cert.pem" BEDROCK_UPDATE_SOURCE_DIR="$test_source" BEDROCK_TEST_NOW=1787600000 \
    "$checker" "$@"
}
run_settings() {
  BEDROCK_UPDATE_TEST_MODE=1 BEDROCK_UPDATE_POLICY_FILE="$work/settings/policy.json" \
...
jq '.generation = 4' "$work/remote/manifest.json" > "$work/remote/manifest.tmp"
