...
jq -n '{schema:1,id:"photos",operation:"install-staged",confirmation:"INSTALL APPLICATION photos"}' > "$work/request.json"; run
[ "$(grep -c ' succeeded ' "$work/task-calls")" -eq 5 ]; grep -Fx 'stop photos STOP APPLICATION photos' "$work/app-calls" >/dev/null; grep -Fx 'start photos START APPLICATION photos' "$work/app-calls" >/dev/null; grep -Fx 'remove photos REMOVE APPLICATION photos' "$work/app-calls" >/dev/null; grep -Fx 'update-latest photos UPDATE APPLICATION photos' "$work/app-calls" >/dev/null; grep -Fx 'install-staged photos INSTALL APPLICATION photos' "$work/app-calls" >/dev/null
