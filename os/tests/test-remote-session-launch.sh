...
BEDROCK_REMOTE_LAUNCH_PRIVATE_KEY="$work/key" BEDROCK_REMOTE_LAUNCH_LOG="$work/log" \
  "$tool" "$device" "$session"
...
if BEDROCK_REMOTE_LAUNCH_TEST_MODE=1 BEDROCK_REMOTE_LAUNCH_SYSTEMD_RUN="$work/systemd-run" BEDROCK_REMOTE_LAUNCH_PRIVATE_KEY="$work/key" "$tool" '../bad' "$session" >/dev/null 2>&1; then
