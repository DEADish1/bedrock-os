...
#!/bin/sh
printf '%s\n' "$*" >> "$BEDROCK_TEST_SESSION_LOG"
EOF
chmod +x "$work/systemctl"
terminator="$ROOT/os/config/includes.chroot/usr/lib/bedrock/terminate-remote-device-sessions"
if env BEDROCK_REMOTE_SESSION_TEST_MODE=1 BEDROCK_REMOTE_SESSION_SYSTEMCTL="$work/systemctl" BEDROCK_TEST_SESSION_LOG="$work/sessions.log" "$terminator" invalid >/dev/null 2>&1; then echo "invalid device session target accepted" >&2; exit 1; fi
envs="BEDROCK_PAIRING_TEST_MODE=1 BEDROCK_PAIRING_STATE_DIR=$work BEDROCK_PAIRING_TEST_WALL_NOW=1000 BEDROCK_PAIRING_SESSION_TERMINATOR=$ROOT/os/config/includes.chroot/usr/lib/bedrock/terminate-remote-device-sessions BEDROCK_REMOTE_SESSION_TEST_MODE=1 BEDROCK_REMOTE_SESSION_SYSTEMCTL=$work/systemctl BEDROCK_TEST_SESSION_LOG=$work/sessions.log"
...
grep -Fx "stop bedrock-remote-device@$device.target" "$work/sessions.log" >/dev/null
