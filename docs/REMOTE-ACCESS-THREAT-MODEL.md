...
`manage-remote-devices` lists privacy-bounded device metadata without keys or fingerprints, renames devices, sets a bounded expiry, and irreversibly revokes them using exact confirmation. Revocation is committed before synchronously stopping the device's `bedrock-remote-device@DEVICE.target`; every future transport session must bind its service to that target so revocation terminates all live sessions. Failure to stop the target is surfaced without restoring trust. The authenticated management interface exposes sanitized pending-request identifiers and guarded, idempotent approval plus trusted-device lifecycle controls; mutations synchronously refresh the public status feed. A bounded, rate-limited Unix-socket gateway now exposes request and redemption only to the dedicated remote-transport identity, returns indistinguishable failures, and keeps codes and client keys out of process arguments. An idempotent initializer provisions and verifies the server X25519 identity before the gateway starts. Relay ingress, session-service binding, and the Noise transport handshake remain open, so the roadmap pairing and device-lifecycle items are not yet complete. See the [remote pairing gateway contract](REMOTE-PAIRING-GATEWAY.md).
