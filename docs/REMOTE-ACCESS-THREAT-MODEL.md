...
The relay-facing hop additionally requires TLS 1.3, with 0-RTT disabled, bounded records, hostname verification, normal platform trust, and the exact `bedrock-relay/1` ALPN. TLS protects routing metadata on each hop but is not the end-to-end security boundary. TLS 1.3 is defined by [RFC 8446](https://datatracker.ietf.org/doc/html/rfc8446). Static certificate pinning is not required for the relay: OWASP cautions that pinning adds rotation risk unless a specific threat model and safe out-of-band pin lifecycle justify it. The paired Noise static key is the server identity pin. The implemented [outbound relay connector](REMOTE-RELAY.md) never listens on a network socket and multiplexes at most 16 opaque streams over the verified TLS connection.
...
`manage-remote-devices` lists privacy-bounded device metadata without keys or fingerprints, renames devices, sets a bounded expiry, and irreversibly revokes them using exact confirmation. Revocation is committed before synchronously stopping the device's `bedrock-remote-device@DEVICE.target`; every authorized transport stream runs as a unique transient service with `PartOf` set to that target, so stopping it terminates those streams. The launcher passes the root-only server key as a systemd credential to an unprivileged service restricted to Unix sockets, and the responder requires the encrypted authorization to name the same device encoded in the unit relationship. It also creates a unique short-lived local API token, passes it only as a service credential, and revokes and removes it on exit; remote peers cannot inject or observe bearer authorization. The unprivileged outbound TLS connector can submit only one connected AF_UNIX stream descriptor and bounded opaque route metadata to a kernel-peer-authenticated root broker. The broker never parses stream data, accepts canonical identifiers only, caps concurrency, and launches only the fixed pairing or device helper; pairing services receive neither API access nor an API credential. Failure to stop a target is surfaced without restoring trust. The authenticated management interface exposes sanitized pending-request identifiers and guarded, idempotent approval plus trusted-device lifecycle controls; mutations synchronously refresh the public status feed. A bounded, rate-limited Unix-socket gateway exposes request, redemption, and exact paired-key session authorization only to the dedicated remote-transport identity, returns indistinguishable failures, and keeps codes and client keys out of process arguments. Successful authorization rejects expired or revoked devices, records last-seen time, and starts the device target. An idempotent initializer provisions and verifies the server X25519 identity before the gateway starts. A bounded connected-stream responder completes Noise XX with that server identity and derives the gateway client key only from the authenticated handshake. After authorization it forwards only bounded versioned API records through the existing authenticated Unix API, enforces age/record/directional-byte limits, and rekeys at a fixed record boundary. Production relay provisioning, end-to-end client acceptance, and independent review remain open, so the roadmap pairing and device-lifecycle items are not yet complete. See the [remote pairing gateway contract](REMOTE-PAIRING-GATEWAY.md).
