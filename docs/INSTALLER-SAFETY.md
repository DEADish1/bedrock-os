...
Linux now has a native physical-device adapter that keeps the same exclusive file handle through streaming write, reread verification, cache synchronization, and finalization. That branch is omitted unless the build receives the exact `BEDROCK_ENABLE_PHYSICAL_WRITER=I_ACCEPT_REAL_DEVICE_DATA_LOSS` token, production trust mode, and a Linux target. Missing production trust, a different token, or Windows/macOS targets fail the build or retain the non-writing path. Normal builds test that the gate is disabled. The protected protocol also distinguishes the existing preflight-only exit from a genuine completed write.

Current CI and ordinary release builds close the exclusive probe immediately and still stop with the dedicated preflight-only exit code. No physical bytes are written. The gated Linux connection is for controlled disposable-drive acceptance only until genuine evidence exists; Windows and macOS remain unconnected.
