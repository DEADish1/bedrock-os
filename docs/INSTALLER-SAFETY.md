...
Progress now has a versioned, session-bound, monotonic contract with bounded byte counts and explicit preparing, approval, writing, reread-verification, finalization, completion, and failure phases. The native pipeline reports real written and reread byte counts and the graphical shell no longer displays a fabricated fixed percentage. App-local preparation, approval, completion, and failure events are connected. Privileged byte-progress relay remains intentionally disconnected until each elevation transport has an authenticated read-only channel; progress-delivery failure must never interrupt or falsely complete a physical write.
