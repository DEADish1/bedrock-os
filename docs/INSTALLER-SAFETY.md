...
The desktop app now serializes one bounded request and starts the installed helper without a shell. Linux uses the exact `/usr/bin/pkexec /usr/bin/bedrock-media-writer` pair covered by the installed policy. Windows uses the native `runas` verb, waits on the returned process handle, and accepts only the helper's dedicated preflight-only exit code. Requests are base64-encoded only to preserve exact argument boundaries; they are not treated as secret or trusted until the elevated helper decodes and validates them. macOS remains explicitly unavailable until the signed SMAppService/XPC peer channel is implemented.

The helper still stops after preflight with its dedicated non-success exit code. It does not open a physical target until administrator identity checks, whole-device opening, exclusive access, flush/eject, and real-device acceptance tests are complete.
