...
The helper closes that exclusive probe immediately and still stops with its dedicated non-success exit code. No bytes are written, and the native media engine remains disconnected until flush/eject handling and real-device acceptance tests are complete.
