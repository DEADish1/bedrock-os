...
The helper closes its current exclusive probe immediately and still stops with its dedicated non-success exit code. No bytes are written, and the finalization layer is not invoked on a physical device until the native media engine is connected and real-device acceptance tests are complete.
