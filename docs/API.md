...
The mutation-side privilege boundary is packaged separately from the API. A root action broker listens only on `/run/bedrock-action-broker/action.sock`, authenticates the kernel-reported peer as the `bedrock-api` account, accepts a strict bounded schema, and maps update-policy choices to fixed arguments for `bedrock-update-settings` without invoking a shell. Pending and terminal request UUIDs, request hashes, and creation times are retained in a bounded owner-only ledger so interruption recovery is safe, identical successful retries are replayed, and conflicting reuse fails closed. Each policy change publishes a privacy-safe `update-policy` task and exactly one terminal audit event. The broker never returns helper output.
