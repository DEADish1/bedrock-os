...
This gateway completes the server-side bootstrap and device-authorization boundary only. Remote transport still must implement the approved Noise protocol, prove possession before calling authorization, join the returned service target, and pass the full transport acceptance matrix before pairing can be marked complete.
