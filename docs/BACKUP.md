...
The hourly timer runs due plans and applies daily, weekly, and monthly retention only after a successful backup and verified snapshot listing. Administrators can also run a plan with `RUN ENCRYPTED BACKUP PLAN`. Restore requires a full snapshot identity and the exact phrase `RESTORE BACKUP PLAN SNAPSHOT ID`; data is restored into a new `/srv/bedrock/restores/PLAN` directory and never overwrites an existing restore.
