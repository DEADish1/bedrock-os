...
`write-protected-install.sh` does not accept test substitutions. It reruns the independent protected preflight immediately before handing the fixed live-media artifact and freshly confirmed target to the root-owned writer. A successful result deliberately reports `layout_finalized: false`: GPT relocation, persistent-state expansion, final whole-layout verification, and real disposable-system-disk acceptance are not connected yet. Therefore this checkpoint does not complete the v0.3 on-server installation item, and CI compiles but never executes the gated physical path.
