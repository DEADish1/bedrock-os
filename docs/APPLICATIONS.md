...
Use `bedrock-apps list`, `check-updates`, `start`, `stop`, `install`, `update`, and `remove`. The authenticated management interface exposes state-aware start and graceful-stop controls with exact confirmation and idempotent replay protection; it receives only the privacy-safe running state. Start, stop, install, update, and removal are blocked during maintenance mode. A catalog and automatic third-party trust decisions remain outside this runtime boundary.
