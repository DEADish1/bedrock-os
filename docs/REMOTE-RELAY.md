...
`/usr/share/bedrock/remote/relay-config.example.json` documents the strict configuration fields. The production relay endpoint and authorization credential are deployment inputs, not source-controlled defaults. End-to-end acceptance still requires that endpoint plus a production client; the repository test uses a temporary local CA, requires TLS 1.3 and the exact ALPN, validates registration and routing, and proves opaque bytes traverse the descriptor handoff unchanged.
