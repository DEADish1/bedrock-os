...
The authenticated Settings interface exposes this guarded boundary with strong state preconditions and idempotent requests. The privileged broker stages the submitted token in a root-only request file, invokes the helper without placing secrets in process arguments, and removes the staging file. The public settings feed reveals only whether a relay is configured.
