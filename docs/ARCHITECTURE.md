...
- Updates: signed release metadata, user-controlled scheduled or manual availability checks, immutable stable/beta endpoints, resumable verified image download, inactive-slot installation, health-confirmed promotion, and automatic/manual rollback. Scheduled checks default off until first-run setup records the user's choice, never install automatically, and make no network request when disabled. Stable rejects prerelease versions; beta requires an explicit risk acknowledgement. UKIs carry monotonic `IMAGE_VERSION` metadata; systemd-boot selects the newest viable image and moves an entry behind its fallback when its counted attempts reach zero.
