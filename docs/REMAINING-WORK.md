...
- [ ] **0.6.3 — Task and audit coverage:** Confirm every long-running or destructive operation emits bounded progress, terminal task state, and privacy-safe audit outcomes, including error/interruption paths. The [source-level coverage audit](TASK-AUDIT-COVERAGE.md) identifies direct image upload/discard as a gap and records remaining per-family validation. Relay configuration/disablement and VM passthrough assignment/removal now emit bounded terminal events, including failed-restart and rejected-running-guest outcomes; [relay validation](https://github.com/DEADish1/bedrock-os/actions/runs/35730263089) and [passthrough validation](https://github.com/DEADish1/bedrock-os/actions/runs/35730587976) passed. This item remains open.
